...
const AssignToBatchModal = dynamic(() => import("./AssignToBatchModal"), {
  ssr: false,
});
// Using shared confirmation modal if easily accessible or dup it. I'll duplicate to be safe cleanly.
const ConfirmationModal = dynamic(
  () => import("../../Admin/ConfirmationModal"),
  {
    ssr: false,
  },
);
...
export default function Materials() {
  ...
  }, [pagination.page, selectedType, user]);
  ...
  const fetchMaterials = async () => {
    ...
    try {
      ...
      // Ideally filter by trainer/uploaded by me.
      // Admin dashboard showed ALL materials. Trainer might only see their own?
      // For now, fetching all as per Admin implementation reference, or if backend filters.
      // If we want ONLY trainer materials, we might need a filter if backend supports it.
      // Assuming 'uploadedBy' field exists or we filter on client.
  ...
  return (
    <div className="space-y-6 p-8">
          ...
          {filteredMaterials.map((material) => (
            ...
            >
              ...
              <div className="flex-1 flex flex-col">
                ...
                {/* Footer Info */}
                <div className="pt-3 border-t border-neutral-200 dark:border-neutral-800 flex items-center justify-between gap-2 text-xs text-neutral-400">
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    className="h-6 text-xs px-2 hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-blue-900/20 dark:hover:text-blue-400"
                    onClick={() =>
                      setAssignModal({
                        isOpen: true,
      ...
      <AssignToBatchModal
        ...
        onClose={() =>
          setAssignModal({ isOpen: false, materialId: "", materialTitle: "" })
        }
        materialId={assignModal.materialId}
        materialTitle={assignModal.materialTitle}
      />
